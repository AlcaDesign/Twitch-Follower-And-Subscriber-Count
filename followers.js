...
function g() {
	...
	function save(f, c, n, j, c2, n2, j2) {
		if(f === 1) fs.writeFile('followers.txt', format({ '%FOLLOWER_COUNT%': c, '%LAST_FOLLOWER%': n, '%CHANNEL%': channel, '%RAW_JSON%': JSON.stringify(j) }, fol_message));
		else if(f === 2) fs.writeFile('subscribers.txt', format({ '%SUBSCRIBER_COUNT%': c, '%LAST_SUBSCRIBER%': n, '%CHANNEL%': channel, '%RAW_JSON%': JSON.stringify(j) }, sub_message));
		else fs.writeFile('both.txt', format({ '%FOLLOWER_COUNT%': c, '%LAST_FOLLOWER%': n, '%SUBSCRIBER_COUNT%': c2, '%LAST_SUBSCRIBER%': n2, '%CHANNEL%': channel, '%RAW_JSON%': JSON.stringify({ follower: j, subscriber: j2 }) }, sub_message)); }
	...
	function fol(data, date, doCallback) {
		var da = JSON.parse(data), end = +new Date, t =	formDateTime(date);
		if(!!da.follows) {
			...
			save(1, da._total, da.follows[0].user.display_name, da);
			if(doCallback) {
				run('subscriptions', sub, getboth && (config.hasOwnProperty('both_message') && config.both_message !== ''), da, function(sd, fd) { save(3, fd._total, fd.follows[0].user.display_name, fd, sd._total, sd.subscribers[0].user.display_name, sd); });
	...
	function sub(data, date, doCallback, callbackData, addlCallback) {
		...
		if(!!da.subscribers) {
			...
			save(2, da._total, da.subscribers[0].user.display_name, da);
			...
			if(da.error == 'Unauthorized') {
				getsub = false;
				console.log('\033[91m[NOTICE] Unauthorized. \033[39mDisabled checking for subscriptions. The channel you\'ve set to check for is not ');
	}	}	}
	if(getfol && getsub) run('follows', fol, true);
	else if(getfol) run('follows', fol);
	else if(getsub) run('subscriptions', sub);
}
if(fs.existsSync('config.json')) {
	var config = null;
	try { config = JSON.parse(fs.readFileSync('config.json')); } catch(e) { console.err(e); }
	if(!!config &&
		...
		config.hasOwnProperty('delay_to')	&& typeof config.delay_to	== 'boolean') {
			...
			if(getfol || getsub) {
				console.log({ client_id: client_id, channel: channel, info: info, messages: { follower: { message: fol_message, outputting: getfol }, subscriber: { message: sub_message, outputting: getfol }, both: { message: both_message, outputting: getboth } }, delay: delay, delaytimeout: delaytimeout });
				g(); if(!delaytimeout) setInterval(g, delay);
			} else { console.error('Please set up the configuration file so that either the follower/subscriber count variable is in the message or "info" is either "fol", "sub", or "both"'); setTimeout('', 10000); }
	} else { console.error('Improper config. Check for errors in the JSON like missing elements and empty strings. You can delete your config.json and rerun the script to create a correctly formatted configuration file.'); setTimeout('', 10000); }
} else fs.writeFile('config.json',
		prettyJSON({ __comment: 'Visit the repository on GitHub for documentation: github.com/AlcaDesign/Twitch-Follower-And-Subscriber-Count/', client_id: 'asdf123asdf123asdf123asdf123asd', oauth_token: '123asdf123asdf123asdf123asdf123', channel: 'channelname', info: 'followers', message_args: ['%CHANNEL%', '%FOLLOWER_COUNT%', '%SUBSCRIBER_COUNT%', '%LAST_FOLLOWER%', '%LAST_SUBSCRIBER%', '%RAW_JSON%'], follower_message: '%FOLLOWER_COUNT% followers. Last follower: %LAST_FOLLOWER%', subscriber_message: '%SUBSCRIBER_COUNT% subscribers. Last subscriber: %LAST_SUBSCRIBER%', both_message:		'%FOLLOWER_COUNT% followers and %SUBSCRIBER_COUNT% subscribers.', delay_sec: 5, delay_to: false
	}), function() { console.error('No config file found. Created one.'); setTimeout('', 3000); });
